...
exports.data = new discord_js_1.SlashCommandBuilder()
    ...
    .setName("name")
    .setDescription("The name of the waifu you want to trade (you can select a copy after).")
    .setRequired(true));
async function execute(interaction) {
    ...
    const initialSelectId = crypto_1.default.randomBytes(6).toString("hex");
    const initialRow = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.StringSelectMenuBuilder()
    ...
    const initialFilter = (i) => i.customId === `trade-waifu-${initialSelectId}`;
    ...
    initialCollector.on("collect", async (initialI) => {
        ...
        initialWaifu = (await models_1.Waifu.findByPk(initialI.values[0]));
        await initialI.deferUpdate();
        const targetSelectWaifuId = crypto_1.default.randomBytes(6).toString("hex");
        const targetSelectWaifuRow = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.StringSelectMenuBuilder()
            .setCustomId(`trade-waifu-${targetSelectWaifuId}`)
            .setPlaceholder("Select a waifu to trade"));
        const waifuList = [];
        targetUserWaifus.forEach((waifu) => {
            if (!waifuList.includes(waifu.name))
                waifuList.push(waifu.name);
        });
        ...
        const targetFilter = (select) => select.customId === `trade-waifu-${targetSelectWaifuId}`;
        const targetCollector = interaction.channel.createMessageComponentCollector({
            componentType: discord_js_1.ComponentType.StringSelect,
            filter: targetFilter,
            time: 30000,
        });
        ...
        targetCollector.on("collect", async (targetI) => {
            ...
            const targetSelectCopyId = crypto_1.default.randomBytes(6).toString("hex");
            const targetSelectCopyRow = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.StringSelectMenuBuilder()
            ...
            targetUserWaifus.sort((a, b) => a.atk - b.atk).splice(25); // discord only allows 25 items
            targetUserWaifus.forEach((waifu) => {
                console.log(`ATK:${waifu.atk} | HP:${waifu.hp} | SPD:${waifu.spd}`);
            ...
            const finalFilter = (i) => i.customId === `trade-waifu-${targetSelectCopyId}`;
            ...
            finalCollector.on("collect", async (finalI) => {
                ...
                targetWaifu = (await models_1.Waifu.findByPk(finalI.values[0]));
